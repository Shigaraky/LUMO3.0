...
const Store = {
    ...
    save() {
        localStorage.setItem('LUMO_V3', JSON.stringify(this.data));
    },
        ...
        if (oldIdx > -1) {
            ...
            if (tx.installments > 1 && tx.type === 'expense') {
                ...
                for (let i = 0; i < tx.installments; i++) {
                    ...
                    const newTx = {
                        ...tx,
                        id: (i === 0 ? tx.id : Utils.genId()),
                        amount: baseAmt,
                        date: nextD.toISOString().split('T')[0],
                        desc: `${tx.desc} (${i + 1}/${tx.installments})`,
                        installments: 1
                    };
    ...
    addCategory(n) {
        if (!this.data.categories.includes(n)) {
    ...
    updateCategory(o, n) {
        ...
        if (i > -1) {
            this.data.categories[i] = n;
            this.data.transactions.forEach(t => { if (t.category === o) t.category = n; });
            this.data.recurring.forEach(r => { if (r.category === o) r.category = n; });
            this.save();
        }
    },
    ...
    deleteCategory(n) {
        ...
        this.data.transactions.forEach(t => { if (t.category === n) t.category = 'Altro'; });
        this.save();
    },
...
const DataMgr = {
    ...
    exportData() {
        try {
            ...
            a.href = url;
            a.download = `LUMO_Backup_${Utils.today()}.json`;
            ...
            alert("Errore Backup File: " + e.message + "\nUsa il Backup Manuale (Copia Testo).");
        }
    },

    // 2. Backup Manuale (Copia Testo)
    manualBackup() {
        const json = JSON.stringify(Store.data, null, 2);
        const h = `
            <p style="font-size:0.9rem; color:var(--text-muted); margin-bottom:10px">Copia questo testo e salvalo nelle tue Note prima di pulire la cache.</p>
            <textarea id="backup-text" style="width:100%; height:200px; padding:10px; border-radius:12px; border:1px solid var(--border); font-family:monospace; font-size:0.7rem;">${json}</textarea>
            <button class="btn-primary" onclick="DataMgr.copyToClipboard()">Copia Tutto</button>
        `;
        UI.openModal("Backup Manuale", h);
    },

    copyToClipboard() {
        const txt = document.getElementById('backup-text');
        txt.select();
        txt.setSelectionRange(0, 99999); // Per mobile
        navigator.clipboard.writeText(txt.value).then(() => {
            alert("Dati copiati negli appunti!");
        }).catch(() => {
            document.execCommand('copy'); // Fallback
            alert("Dati copiati (Fallback)!");
        });
    },
...
const UI = {
    ...
    drawAccounts() {
        ...
        savings.forEach(a => {
            h += `<div class="acc-card acc-savings" onclick="UI.modalAccount('${a.id}')">
                <div><div style="font-weight:700; font-size:1.1rem">${a.name}</div><div style="font-size:0.8rem; opacity:0.6; text-transform:uppercase">Risparmio</div></div>
                <div style="font-size:1.2rem; font-weight:700; color:var(--gold)">${Utils.fmtMoney(a.balance)}</div>
            </div>`;
        });
        ...
        liquid.forEach(a => {
            h += `<div class="acc-card" onclick="UI.modalAccount('${a.id}')">
        ...
        const h = `
            <div class="card" style="margin-top:0">
                <button class="list-item" style="width:100%; border:none; background:none;" onclick="UI.modalCategories()">
                    <div style="font-weight:600">🏷 Gestione Categorie</div><div>›</div>
                </button>
            ...
            <div class="card">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:10px;">
                    <button class="btn-primary" style="background:#475569; margin:0;" onclick="DataMgr.exportData()">📤 Scarica File</button>
                    <button class="btn-primary" style="background:#475569; margin:0;" onclick="DataMgr.manualBackup()">📋 Copia Testo</button>
                </div>
                ...
                <p style="font-size:0.8rem; color:var(--text-muted); margin-top:15px; text-align:center;">LUMO v4.5</p>
            </div>
        `;
        this.openModal('Impostazioni', h);
    },

    // ALTRE MODALI (Tx, Recurring, Account, Categories) SONO INVARIATE MA RIPORTATE PER COMPLETEZZA
    modalTx(id = null) {
...
const Auth = {
    ...
    render() {
        ...
        if (container) {
            container.innerHTML = Array(this.input.length).fill('<div class="pin-dot filled"></div>').join('');
        }
    },
    checkPin() {
        if (!Store.data.pin) {
            if (this.input.length >= 4) {
                Store.data.pin = this.input;
                Store.save();
                this.unlock();
            } else {
                alert('Min 4 cifre');
            ...
            if (this.input === Store.data.pin) {
                this.unlock();
            } else {
                const msg = document.getElementById('auth-msg');
                if (msg) msg.innerText = 'PIN Errato';
                this.clearPin();
            }
        }
    },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Auth.input = '';
    Auth.render();
    document.getElementById('fab-add').addEventListener('click', () => UI.modalTx());
});