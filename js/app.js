...
const DataMgr = {
    exportData() {
        try {
            // 1. Crea il contenuto JSON
            const dataStr = JSON.stringify(Store.data, null, 2);
            // 2. Crea un Blob (File virtuale) - Più sicuro su Mobile
            const blob = new Blob([dataStr], { type: "application/json" });
            const url = URL.createObjectURL(blob);
            
            // 3. Crea link nascosto e cliccalo
    ...
    handleFile(input) {
        ...
        reader.onload = function(e) {
            try {
                ...
                if (json.accounts && json.transactions) {
                    if(confirm("Attenzione: Questo sovrascriverà tutti i dati attuali con quelli del backup. Continuare?")) {
                        Store.data = json;
                        Store.save();
                    ...
                    alert("Il file selezionato non è un backup valido di LUMO.");
                }
            } catch (err) {
        ...
        // Reset input per permettere di ricaricare lo stesso file se serve
        input.value = ''; 
    }
};
    ...
    drawRecurring() {
        ...
        if(Store.data.recurring.length > 0) {
            ...
            sorted.forEach(r => {
                ...
                const isCurrentMonth = (nd.getMonth() === curM && nd.getFullYear() === curY);
                let cssClass = isCurrentMonth ? 'rec-card rec-yellow' : 'rec-card rec-green';
                let status = isCurrentMonth ? `⏳ In arrivo: ${Utils.fmtDate(r.nextDate)}` : `✓ Addebitata (Prox: ${Utils.fmtDate(r.nextDate)})`;
